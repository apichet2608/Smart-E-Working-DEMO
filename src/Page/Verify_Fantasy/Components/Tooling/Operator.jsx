...
function Operator(props) {
  ...
  useEffect(() => {
    if (response_API.status === "OK") {
      setTitle(response_API.data.data[0].tool_haed);
      setMessage(response_API.message);
      setDataCheck([response_API.data.data[0].tool_detile]); // Added this line
      console.log([response_API.data.data[0].tool_detile]); // Added this line
    } else {
      setTitle("OPERATOR");
      setMessage(response_API.message);
    }
      ...
      {dataCheck && dataCheck.length > 0 ? (
        <div>
          {response_API.status === "OK" ? (
            <Chip_Operator
              title={title}
              data={response_API.data.data[0].tool_detile}
              EWK_ID={EWK_ID}
            />
          ) : (
